...
- `optional`: holds a value or nothing, with `nullopt` or empty
...
- `printf`: via WriteConsoleA output, `%d` `%u` `%x` `%X` `%p` `%s` `%c` `%f` `%ll` `%z`
- `mutex`, `lock_guard`: basic mutual exclusion and scoped locking
- `once_flag`, `call_once`: https://en.cppreference.com/cpp/thread/call_once
- `this_thread::yield`: calls NtYieldExecution
- `this_thread::sleep_for`: calls NtDelayExecution
